...
task :install do
	replace_all = false
	Dir['*'].each do |file|
		next if %w[Rakefile README LICENSE vim-pre-pathogen.tar.gz].include? file
...
def destfile(file,show_base = false)
	return show_base ? File.join(show_base ? ENV['HOME'] : '', ".#{file.sub('.erb', '')}") : ".#{file.sub('.erb', '')}"
end
...
def link_file(file)
	if file =~ /.erb$/
		puts "generateing #{destfile(file)}"
